...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running():
        ...
        # —————— Insert here ——————
        # Print each finger’s local‐fz vs. its compressive force n·f
        # ——————————————————————
        ...
        def build_friction_constraints(mu=1.5, ng=6):
            A_blocks = []
            for i, pos in enumerate(contact_positions):
                ...
                for k in range(ng):
                    theta = 2 * np.pi * k / ng
                    tangent = mu * (np.cos(theta) * t1 + np.sin(theta) * t2)
                    s = n + tangent
                    s /= np.linalg.norm(s)  # Normalize the generator
                    s_list.append(s)

                # Construct F matrix from face normals (cross products of adjacent generators)
                F_i = []
                for k in range(ng):
                    s_k = s_list[k]
                    s_next = s_list[(k + 1) % ng]
                    fk = np.cross(s_k, s_next)
                    F_i.append(fk)

                F_i = np.vstack(F_i)  # Shape: (ng, 3)

                # Create block for one contact (affects only 3 of 12 λ components)
                A_contact = np.zeros((ng, 6 * len(contact_positions)))
                A_contact[:, i * 6:i * 6 + 3] = F_i
                A_blocks.append(A_contact)

            A_ineq = np.vstack(A_blocks)  # Shape: (ng*num_contacts, 12)
            return A_ineq

        A_ineq = build_friction_constraints(mu=mu, ng=6)
        c = np.zeros(13)
        c[-1] = -1
