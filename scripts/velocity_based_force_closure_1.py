...
start_time = None
wrench_zero_printed = False
wrench_gravity_printed = False

with mujoco.viewer.launch_passive(model, data) as viewer:
    print("[INFO] Viewer launched. Waiting 2 seconds before starting control loop...")
    time.sleep(2.0)  # Force simulation to stay idle before control logic begins
    ...
    while viewer.is_running():
        ...
        print(f"[DEBUG] Elapsed time: {elapsed_time:.4f} seconds")

        if elapsed_time < 2.0:
            w_ext = np.zeros(6)
            if not wrench_zero_printed:
                print(f"[WRENCH] t={elapsed_time:.2f}s — w_ext set to ZERO.")
                wrench_zero_printed = True
        else:
            w_ext = np.array([0, 0, (-9.81 * box_mass) + 2, 0, 0, 0])
            if not wrench_gravity_printed:
                print(f"[WRENCH] t={elapsed_time:.2f}s — w_ext set to GRAVITY with offset: {w_ext}")
                wrench_gravity_printed = True
        ...
        # Normals (sensor-based or fallback)
        normals = []
        for i, f in enumerate([f_left, f_right]):
            ...
            if norm > 1e-6:
                ...
                print(f"[NORMAL] Sensor force too small at contact {i+1}, using fallback normal: {fallback}")
                n = fallback
            normals.append(n)
        ...
        def build_friction_constraints():
            ...
            for i, Ai in enumerate(A):
                A_block[i * 5:(i + 1) * 5, i * 6:i * 6 + 3] = Ai
            return A_block
